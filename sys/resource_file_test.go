...
func TestRemoteFile_Basic(t *testing.T) {
	var cases = []struct {
		path    string
		content string
		config  string
		...
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			r.UnitTest(t, r.TestCase{
				...
				CheckDestroy: func(*terraform.State) error {
					...
					return errors.New("remote_file did not get destroyed")
				},
			})
		})
	}
...
func TestRemoteFile_Permissions(t *testing.T) {

	randomPath := acctest.RandomWithPrefix("test-file-perms")

	destinationDirPath := "../test/" + randomPath
	destinationFilePath := destinationDirPath + "/remote_file"
